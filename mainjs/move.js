...
$(document).ready(function() {
    ...
    $('.btn1').css('background','#beddfa'); //첫번째 불켜
    $('.btn1').css('width','30').css('border-radius','6px');
    ...
    function moveg(){
     ...
     for(var i=1;i<=imageCount;i++){
          ...
          $('.btn'+i).css('width','12'); //버튼불다꺼!!
      }
      $('.btn'+cnt).css('background','#beddfa');
      $('.btn'+cnt).css('width','30').css('border-radius','6px');;//자신만 불켜                 
   ...
   $('.mbutton').click(function(event){  //각각의 버튼 클릭시
      ...
      $('.btn'+cnt).css('background','#beddfa');
      $('.btn'+cnt).css('width','30').css('border-radius','6px');;//자신 버튼만 불켜 
          if(cnt==imageCount)cnt=0;  //카운트 초기화
